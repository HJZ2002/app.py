...
class LoginWindow(QtWidgets.QWidget):
    ...
    def start_session_timer(self):
        ...
        self.session_timer.timeout.connect(self.check_token_expiry)
        self.session_timer.start(1000)  # Every 1 second
    ...
    def check_token_expiry(self):
        ...
        if datetime.now() >= self.expiry:
            ...
            # 🧾 Notify server of logout BEFORE clearing username
            try:
                requests.post("http://127.0.0.1:5000/logout", json={"username": self.username.text()})
            except:
                pass  # Silent fail if server not reachable
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def update_selected(self):
        ...
        try:
            ...
            new_status, new_transact_type, new_total, new_name, new_remarks, new_contact, kilos, selected_id))
    ...
    def add_data(self):
        ...
        if not name or not contactnum or not transaction_type or kilos <= 0 or not remarks:
            ...
            QMessageBox.warning(self, "Input Error",
                                "Please make sure all fields are filled and Kilos is greater than 0.", QMessageBox.Ok)
        ...
        try:
            ...
            self.cursor.execute('''INSERT INTO LaundryItems (LaundryID, Date, Name, CellNum, TransactionType, Kilos, Total, Status, Remarks)
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                                (laundry_id, current_date, name, contactnum, transaction_type, kilos, total, status,
                                 remarks))
            self.conn.commit()
            ...
            QMessageBox.critical(self, "Database Error", f"An error occurred while saving the data: {str(e)}",
                                 QMessageBox.Ok)
